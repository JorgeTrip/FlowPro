    ...
    <table className="min-w-[1100px] divide-y divide-gray-200 dark:divide-gray-800 text-left text-[11px]">
      <thead className="bg-gray-50 dark:bg-[#2C2C2E] text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider select-none">
        <tr className="divide-x divide-gray-100 dark:divide-gray-800">
          {/* PT Sincronizados */}
          <th onClick={() => solicitarOrdenTercerizados('codigoPT')} className={`${clsSincronizadoHeader} px-2.5 py-2 border-r border-gray-150 dark:border-gray-800 cursor-pointer`}>Código PT{getIndT('codigoPT')}</th>
          <th onClick={() => solicitarOrdenTercerizados('descripcionPT')} className={`${clsSincronizadoHeader} px-2.5 py-2 border-r border-gray-150 dark:border-gray-800 cursor-pointer`}>Descripción PT{getIndT('descripcionPT')}</th>
          <th onClick={() => solicitarOrdenTercerizados('stockPTEntreRios')} className={`${clsSincronizadoHeader} px-2.5 py-2 text-right border-r border-gray-150 dark:border-gray-800 cursor-pointer`}>Stock PT E.R.{getIndT('stockPTEntreRios')}</th>
          <th onClick={() => solicitarOrdenTercerizados('stockPTCABA')} className={`${clsSincronizadoHeader} px-2.5 py-2 text-right border-r-2 border-gray-300 dark:border-gray-700 cursor-pointer`}>Stock PT CABA{getIndT('stockPTCABA')}</th>
      ...
      <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-[#1C1C1E] text-gray-900 dark:text-gray-300">
        {resultadosFiltradosTercerizados.map((fila, idx) => (
          <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-[#2C2C2E]/40 transition-colors divide-x divide-gray-100 dark:divide-gray-850 text-xs align-top">
            <td className={`${clsSincronizadoCell} px-2.5 py-2 font-mono font-semibold`}>{fila.codigoPT}</td>
            <td className={`${clsSincronizadoCell} px-2.5 py-2`}>
              <div className="truncate max-w-[250px] text-ellipsis">{fila.descripcionPT}</div>
            </td>
            
            <td className={`${clsSincronizadoCell} px-2.5 py-2 text-right font-mono`}>{(fila.stockPTEntreRios ?? 0).toFixed(1)}</td>
            <td className={`${clsSincronizadoCell} px-2.5 py-2 text-right font-mono border-r-2 border-gray-300 dark:border-gray-700`}>{(fila.stockPTCABA ?? 0).toFixed(1)}</td>
