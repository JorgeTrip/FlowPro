...
interface DropdownLineaProps {
  lineasFiltradas: string[];
  setLineasFiltradas: (lineas: string[]) => void;
  lineasDisponibles: string[];
}

export default function DropdownLinea({
  lineasFiltradas,
  setLineasFiltradas,
  lineasDisponibles,
}: DropdownLineaProps) {
  ...
  const toggleLinea = (l: string) => {
    if (lineasFiltradas.includes(l)) {
      setLineasFiltradas(lineasFiltradas.filter((x) => x !== l));
    } else {
      setLineasFiltradas([...lineasFiltradas, l]);
    }
  };
  ...
  const cantidadActivos = lineasFiltradas.length;
  ...
  return (
    <div ref={refContenedor} className="relative inline-block text-left">
      ...
      {abierto && (
        <div className="absolute left-0 mt-1.5 w-60 rounded-xl bg-white dark:bg-[#1C1C1E] border border-gray-250 dark:border-gray-800 shadow-lg p-3 z-30 space-y-2 transition-all animate-in fade-in slide-in-from-top-1 duration-100 max-h-64 overflow-y-auto">
          <h3 className="text-xs font-bold text-gray-700 dark:text-gray-200 mb-1">Filtrar por Línea</h3>
          <div className="space-y-1.5">
            {lineasDisponibles.length === 0 ? (
              <span className="text-[11px] text-gray-400 block p-1">No hay líneas cargadas</span>
            ) : (
              lineasDisponibles.map((linea) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={lineasFiltradas.includes(linea)}
                  ...
                  />
                  <span className="font-medium select-none text-[11px] truncate max-w-[190px]" title={linea}>{linea}</span>
                </label>
              ))
            )}
          </div>
