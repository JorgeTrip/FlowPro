    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-gray-800 shadow-sm">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Arrastra un único libro de Excel que contenga solapas separadas para recetas, stock e histórico de consumos.
        </p>
        <FileUpload
        ...
        {hojasDisponibles.length > 0 && (
          <div className="mt-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-lg bg-gray-50 dark:bg-[#2C2C2E]/40 border border-gray-100 dark:border-gray-800">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase">Hoja de Recetas/Fórmulas</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase">Hoja de Stock por Depósito</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase">Hoja de Consumo Mensual *</label>
            ...
            <div className="flex flex-wrap gap-4 justify-center py-2 px-4 bg-blue-50/30 dark:bg-blue-950/10 border border-blue-100/30 dark:border-blue-900/20 rounded-xl text-xs font-bold">
              <div className="flex items-center space-x-1.5">
                ...
                <span className="text-gray-700 dark:text-gray-300">Recetas: {store.datosCrudosFormulas.length} filas</span>
              </div>
              <div className="flex items-center space-x-1.5">
                <span className={`w-2.5 h-2.5 rounded-full ${store.datosCrudosStock.length > 0 ? 'bg-green-500' : 'bg-red-500'}`} />
                <span className="text-gray-700 dark:text-gray-300">Existencias: {store.datosCrudosStock.length} filas</span>
              </div>
              <div className="flex items-center space-x-1.5">
                <span className={`w-2.5 h-2.5 rounded-full ${store.datosCrudosConsumo.length > 0 ? 'bg-green-500' : 'bg-red-500'}`} />
                <span className="text-gray-700 dark:text-gray-300">Consumos: {store.datosCrudosConsumo.length} filas</span>
              </div>
            </div>
          </div>
        )}
