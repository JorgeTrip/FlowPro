...
interface DropdownPlantaProps {
  plantasFiltradas: string[];
  setPlantasFiltradas: (plantas: string[]) => void;
}

export default function DropdownPlanta({
  plantasFiltradas,
  setPlantasFiltradas,
}: DropdownPlantaProps) {
  ...
  const togglePlanta = (p: string) => {
    if (plantasFiltradas.includes(p)) {
      setPlantasFiltradas(plantasFiltradas.filter((x) => x !== p));
    } else {
      setPlantasFiltradas([...plantasFiltradas, p]);
    }
  };
  ...
  const opciones = [
    ...
    { id: 'ENTRE RIOS', etiqueta: 'Entre Ríos' },
    { id: 'CABA + ENTRE RIOS', etiqueta: 'CABA + Entre Ríos' },
    { id: 'TERC. CABA', etiqueta: 'Tercerizado CABA' },
    { id: 'TERC. ENTRE RIOS', etiqueta: 'Tercerizado Entre Ríos' },
    { id: 'TERC. CON PROV. MP', etiqueta: 'Tercerizado c/ Prov. MP' },
  ];

  const cantidadActivos = plantasFiltradas.length;
  ...
  return (
    <div ref={refContenedor} className="relative inline-block text-left">
      ...
      {abierto && (
        <div className="absolute left-0 mt-1.5 w-56 rounded-xl bg-white dark:bg-[#1C1C1E] border border-gray-250 dark:border-gray-800 shadow-lg p-3 z-30 space-y-2 transition-all animate-in fade-in slide-in-from-top-1 duration-100">
          <h3 className="text-xs font-bold text-gray-700 dark:text-gray-200 mb-1">Filtrar por Planta</h3>
          <div className="space-y-1.5">
            {opciones.map((op) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={plantasFiltradas.includes(op.id)}
                  onChange={() => togglePlanta(op.id)}
