...
/**
 * Función para autodetectar qué columna coincide con las palabras clave provistas.
 */
...
export default function ConfigStep() {
  ...
  // Estados locales temporales del mapeo
  ...
  // Autodetección al inicializar el componente basado en los encabezados reales de Tango
  useEffect(() => {
  ...
  }, [store.columnasFormulas, store.columnasStock, store.columnasConsumo]);
  ...
  const formulasReady =
    mapeoLocal.formulas.codigoProducto &&
    mapeoLocal.formulas.codigoComponente &&
    mapeoLocal.formulas.cantidad;

  const stockReady =
    mapeoLocal.stock.codigoProducto &&
    mapeoLocal.stock.deposito &&
    mapeoLocal.stock.stockFisico;

  const consumoReady =
    mapeoLocal.consumo.codigoProducto &&
    mapeoLocal.consumo.cantidadConsumida;

  const listoParaImportar = formulasReady && stockReady && consumoReady;
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
          <h3 className="text-base font-bold text-gray-800 dark:text-white flex items-center">
            <span className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xs mr-2 font-bold font-mono">1</span>
            Mapeo de Fórmulas / Recetas (BOM)
          </h3>

          <div className="space-y-4">
            <div className="p-4 rounded-xl bg-gray-50/50 dark:bg-[#2C2C2E]/20 border border-gray-100 dark:border-gray-850 space-y-3">
              <h4 className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Producto Terminado (PT)</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <SelectorMapeo label="Cód. Prod. Principal" columnas={store.columnasFormulas} value={mapeoLocal.formulas.codigoProducto} onChange={(v) => setMapeoLocal((prev) => ({ ...prev, formulas: { ...prev.formulas, codigoProducto: v } }))} requerido />
                <SelectorMapeo label="Desc. Prod. Principal" columnas={store.columnasFormulas} value={mapeoLocal.formulas.descripcionProducto} onChange={(v) => setMapeoLocal((prev) => ({ ...prev, formulas: { ...prev.formulas, descripcionProducto: v } }))} />
            ...
            <div className="p-4 rounded-xl bg-gray-50/50 dark:bg-[#2C2C2E]/20 border border-gray-100 dark:border-gray-850 space-y-3">
              <h4 className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Insumos / Materias Primas</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        {/* Columna Derecha: Stock y Consumo */}
        <div className="space-y-6">
          {/* Bloque Stock */}
          <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
            <h3 className="text-base font-bold text-gray-800 dark:text-white flex items-center">
              <span className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xs mr-2 font-bold font-mono">2</span>
              Mapeo de Existencias de Stock
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
              ...
              <SelectorMapeo label="U.M. Control Stock" columnas={store.columnasStock} value={mapeoLocal.stock.unidadMedida} onChange={(v) => setMapeoLocal((prev) => ({ ...prev, stock: { ...prev.stock, unidadMedida: v } }))} />
              <SelectorMapeo label="Cantidad a Recibir" columnas={store.columnasStock} value={mapeoLocal.stock.cantidadARecibir} onChange={(v) => setMapeoLocal((prev) => ({ ...prev, stock: { ...prev.stock, cantidadARecibir: v } }))} />
            </div>
          ...
          <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
            <h3 className="text-base font-bold text-gray-800 dark:text-white flex items-center">
              <span className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xs mr-2 font-bold font-mono">3</span>
              Mapeo de Consumo Mensual (Rotación)
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
      ...
      {store.error && (
        <div className="p-3 bg-red-100 border border-red-200 text-red-700 rounded-lg text-sm">{store.error}</div>
      )}
