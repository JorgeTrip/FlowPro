...
export function IrregularitiesModal({
  ...
  // Extraer items de irregularidades con la línea completa de la planilla original
  ...
  const handleGuardarNota = async (docId: string, filaId: string, nuevaNota: string) => {
    setGuardando(true);
    try {
      await actualizarFilaEnDocumento(docId, filaId, {
        notaIrregularidad: nuevaNota.trim() ? nuevaNota.trim().toUpperCase() : null,
        esIrregular: Boolean(nuevaNota.trim()),
      });
      onActualizado();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fade-in">
      <div className="flex max-h-[90vh] w-full max-w-4xl flex-col rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-800 dark:bg-[#1C1C1E]">
        ...
        {/* Listado de Irregularidades con Líneas Completas */}
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {items.length === 0 ? (
            <div className="py-12 text-center text-sm text-gray-500 dark:text-gray-400">
              No hay irregularidades registradas {filtroEmp ? `para ${filtroEmp}` : 'en el período seleccionado'}.
            </div>
          ) : (
            items.map((item) => (
              <IrregularityRowCard
