...
interface ControlesTablaRubroProps {
  ...
  handleRubroToggle: (r: string) => void;
  seleccionarTodosRubros: () => void;
  limpiarRubros: () => void;
...
export const ControlesTablaRubro: React.FC<ControlesTablaRubroProps> = ({
  modoVista,
  setModoVista,
  meses,
  mesesSeleccionados,
                ...
                {todosLosRubros.map((rubro) => (
                  <label key={rubro} className="flex items-center mb-2 cursor-pointer">
                    <input
                      ...
                      className="mr-2"
                    />
                    <span className="text-sm text-gray-900 dark:text-white">{rubro}</span>
                  </label>
                ))}
              </div>
            </div>
