...
import { EtiquetaBarraRubro } from './EtiquetaBarraRubro';

interface GraficoComparativoRubroProps {
...
export const GraficoComparativoRubro: React.FC<GraficoComparativoRubroProps> = ({
  data,
  activeMetric,
  mesesSeleccionados,
  mostrarVariacion,
          ...
          {payload.map((entry, index) => (
            <p key={index} className="text-sm flex items-center gap-2 mb-1">
              ...
              <span className="text-gray-800 dark:text-white">
                {activeMetric === 'importe' ? formatCurrency(entry.value) : formatQuantity(entry.value)}
              </span>
            </p>
          ))}
        </div>
    ...
    <ResponsiveContainer width="100%" height={chartHeight}>
      ...
      >
        ...
        <XAxis
          ...
          domain={[0, maxValue]}
          tickFormatter={(value) =>
            activeMetric === 'importe' ? formatCurrency(value as number) : formatQuantity(value as number)
          }
          tickLine={false}
          axisLine={false}
