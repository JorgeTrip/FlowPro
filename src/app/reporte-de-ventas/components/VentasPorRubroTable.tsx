...
export const VentasPorRubroTable = ({ ventasPorRubro, cantidadesPorRubro }: VentasPorRubroTableProps) => {
    ...
    const mesesConDatos = useMemo(() => {
        ...
        Object.entries(ventasPorRubro).forEach(([mes, rubros]) => {
            const hasData = Object.values(rubros).some(r => (r.A || 0) > 0 || (r.X || 0) > 0);
    ...
    const datosProcesados = useMemo(() => {
        if (modoVista === 'acumulado') {
                ...
                rubrosSeleccionados.forEach(rubro => {
                    ...
                    if (mapaRubros[rubro]) {
                        ...
                        if (index > 0 && mostrarVariacion) {
                            ...
                            if (valorAnterior > 0) {
                                mapaRubros[rubro].variaciones[mes] = ((valorActual - valorAnterior) / valorAnterior) * 100;
                            }
                        }
                    }
                });
            ...
            // Eliminar rubros sin datos (opcional, pero mejor para limpiar la tabla)
    ...
    }, [ventasPorRubro, cantidadesPorRubro, rubrosSeleccionados, mesesSeleccionados, mostrarCantidad, ordenAscendente, modoVista, mostrarVariacion]);
    ...
    const totales = useMemo(() => {
        if (modoVista === 'acumulado') {
            ...
            // Totales para comparativo (por mes)
    ...
    const exportarDatos = () => {
        if (modoVista === 'acumulado') {
            ...
            exportToExcel([headers, ...rows], `ventas-por-rubro-${mostrarCantidad ? 'cantidad' : 'importe'}`, 'Ventas Por Rubro');
        } else {
            ...
            if (mostrarTotales) {
                ...
                mesesSeleccionados.forEach((mes, idx) => {
                    ...
                    if (mostrarVariacion && idx > 0) row.push('-'); // No calculamos var del total aquí por simplicidad, o podríamos
                });
                row.push(t.granTotal);
                rows.push(row);
            }
            
            exportToExcel([headers, ...rows], `ventas-por-rubro-comparativo`, 'Ventas Por Rubro Comparativo');
        }
    };
